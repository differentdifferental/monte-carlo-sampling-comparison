function convergence_results = run_convergence_experiment(params)
...
for m_idx = 1:n_methods
    ...
    for s_idx = 1:n_sizes
        ...
        if strcmp(method, 'Metropolis')
            ...
            [samples, exec_time] = metropolis_simple(mcmc_total, T, burn_in);
        else
            % 使用eval函数调用对应的方法
            switch method
                case 'BoxMuller'
                    [samples, exec_time] = box_muller_fixed(n, T);
                case 'Rejection'
                    [samples, exec_time, ~] = rejection_sampling_simple(n, T);
                case 'CLT'
                    [samples, exec_time] = clt_approximation_simple(n, T);
            end
        end
